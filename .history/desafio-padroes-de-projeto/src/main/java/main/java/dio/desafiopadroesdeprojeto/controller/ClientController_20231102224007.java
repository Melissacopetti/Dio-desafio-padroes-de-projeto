...
import org.springframework.web.context.support.ServleContextAttributeExporter;
...
public ResponseEntity<ResponseEntity<Clients>> createClient(@RequestBody ClientDTO newClient) {
    try {
        ...
        HttpServletResponse response = ServletContextAttributeExporter.getWebApplicationContext(this.getClass().getClassLoader()).getServletContext().getRequestDispatcher("/").getResponse();
